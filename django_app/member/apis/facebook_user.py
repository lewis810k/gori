...
def fb_complete_login(request, app, token):
    ...
    # username이 기존 데이터베이스에 존재하는지 검사하여 에러메시지 출력
    # usernames = User.objects.values('username')
    # for username in usernames:
    #     if extra_data['id'] in username.values():
    #         # user = User.objects.get(username=extra_data['id'])
    #         user = authenticate(username=extra_data['id'], password='facebook')
    #         # user = authenticate(username='qwer111111@gmail.com', password='facebook')
    #
